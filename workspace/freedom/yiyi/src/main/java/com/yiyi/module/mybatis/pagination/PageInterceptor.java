...
import com.yiyi.module.mybatis.pagination.Dialect.DialectType;
 ...
 * @Modifier @date @Content majun 2014-09-28 新增
 */
...
public class PageInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		if (mappedStatement.getId().matches(pageSqlId)) {
			...
			if (parameterObject == null)
				...
				boolean pageCountFlag = Boolean.valueOf(paramMap.get(
						"pageCountFlag").toString());
				if (pageCountFlag)
					resetPageParameter(sql, connection, mappedStatement,
							boundSql, page);
			}
		}
	...
	private String constructPageSql(String sql, Page<?> page) {
		try {
			...
			DialectType dialectType = DialectType
					.valueOf(dialect.toUpperCase());
			if (dialectType == null)
				throw new IllegalArgumentException(
						"dialect configuration error!");
	 ...
	 *            分页对象
	 */
	private void resetPageParameter(String sql, Connection connection,
		...
		try {
			...
			setParameter(countStmt, mappedStatement, countBS,
					boundSql.getParameterObject());
			...
			// 总页数
			long totalPage = Page.computeTotalPage(totalCount,
					page.getPageSize());
	 ...
	 *             SQL异常
	 */
	private void setParameter(PreparedStatement ps,
